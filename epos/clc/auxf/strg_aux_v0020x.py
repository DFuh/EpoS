...
class STRG():
    ...
    def __init__(self,simu_obj,T0=None, p0=None): #height=300,V=500e3,p0=40e5,T0=317.15):
    ...
    def clc_state_mstp(self, T_in, p_in, t_span, dm_in, dm_out, max_step=np.inf,
        ...
        #print('m_dot[:20,0]: ', m_dot[:20,0])
        #print('results.t[:20]: ', results.t[:20])
        #print('results.y[1,:20]', results.y[1,:20])
        #print(' --- (clc_state_mstp) extract results')
        #print('m_dot[-20:,0]: ', m_dot[-20:,0])
        #print('results.t[-20:]: ', results.t[-20:])
        #print('results.y[1,-20:]', results.y[1,-20:])
        # print(' --- (clc_state_mstp) extract results')
        if simu_obj is not None:
            simu_obj.logger.info(' --- (clc_state_mstp) extract results: rho,u')
        else:
            print(' --- (clc_state_mstp) extract results: rho,u')

        # rho = np.interp(m_dot[:,0], results.t, results.y[1,:])    #kg/m^3
        # u = np.interp(m_dot[:,0], results.t, results.y[0,:])      #J/kg

        rho = results.y[1,:]
        u = results.y[0,:]

        if simu_obj is not None:
            simu_obj.logger.info(' --- (clc_state_mstp) extract results: T,p,...')
        else:
            print(' --- (clc_state_mstp) extract results: T,p,...')

        T = np.array([self.EoS.temperature(x,y) for x,y in zip(rho,u)])    #K
        # print('len(rho): ', len(rho), type(rho))
        # print('len(u): ', len(u), type(u))
        # T = self.EoS.temperature(rho,u) # for x,y in zip(rho,u)])    #K
        p = np.array([self.EoS.pressure(x,y) for x,y in zip (T,rho)])    #Pa
        # p = self.EoS.pressure(T,rho) # for x,y in zip (T,rho)])    #Pa
        t = results.t           #h (?)
    ...
    def ODE_mstp(self,t,Y,T_in,m_dot,c,tcorr):
        ...
        m_dot_in = scpin.interp1d(m_dot[:,0],m_dot[:,1],kind='nearest',fill_value=(m_dot[0,1],m_dot[-1,1]),bounds_error=False)
        # m_dot_out = scpin.interp1d(m_dot[:,0],m_dot[:,2],kind='nearest',fill_value=(m_dot[0,2],m_dot[-1,2]),bounds_error=False)
        m_dot_out = scpin.interp1d(m_dot[:,0],m_dot[:,2],kind='nearest',fill_value=(m_dot[0,1],m_dot[-1,1]),bounds_error=False)
        ...
        dudt = (((self.EoS.enthalpy(T_in,rho_in)*m_dot_in(t)*tcorr
                 ...
                 -self.u*(m_dot_in(t)*tcorr-m_dot_out(t)*tcorr)
                 # +float(c)*self.Heat()*3600)/self.m  )
                 +float(c)*self.Heat()*3600)/self.m  )
        drhodt = (m_dot_in(t)*tcorr-m_dot_out(t)*tcorr)/self.V
...
def fill_strg(m0, m_max, m_dot_diff, time_step):
    ...
    return m_act[:], np.divide(dm_grid[:], time_step, where=time_step!=0), np.divide(dm_cvrn[:], time_step, where=time_step!=0)
# ==============================================================================
# ==============================================================================
# ==============================================================================
