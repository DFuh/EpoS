...
-- edit: P_ext for Compression-Power to Pipeline
-> chnaged inputs to clc_strg_mstp
'''
import numpy as np
import pandas as pd
...
def clc_strg_state_iso(obj, df_lst ):
    ...
    #if df_lst is None:
    #    df_lst = obj.dct_strg_extclc['dfs']
    lssim = len(df_lst)
    lst_out = []
    for n,df in enumerate(df_lst):
        ...
        obj.logger.info('(Simu %s / %s) T_in = %s || p_in = %s ', str(n+1),str(lssim),T0_in, p0_in)
        try:
        # if True:
...
def clc_strg_sngl(obj, full_df, T0_in, p0_in):
    ...
    p_in = obj.p.cathode #101325 # Pressure of Electrolyzer output // in Pa
    ...
    dm_prd = full_df.n_H2_ca.to_numpy() * obj.pec.M_H2*obj.pstrg.fctr_scl_input # kg/h (plant level)
    dm_cns = full_df.dm_H2_dmnd.to_numpy()                                      # kg/h
    ...
    if len(l) == 3:
        ...
        m_strg_sc = full_df.m_strg_sc.to_numpy()                                # kg
        m_dot_H2_grid = full_df.m_dot_H2_grid.to_numpy()                        #
        m_dot_H2_strg = full_df.m_dot_H2_strg.to_numpy()
        # m_dot_H2_prd = full_df.n_H2_ca.to_numpy() * obj.pec.M_H2*obj.pstrg.fctr_scl_input
    else:
        obj.logger.info(' --- Run fill_strg --- ')

        (m_strg_sc,
        m_dot_H2_grid,
        m_dot_H2_strg) =  xstrg.fill_strg(storage.m - storage.m_min, storage.m_max-storage.m_min, (dm_prd-dm_cns), tdiff)
    ...
    max_stp = getattr(obj.pstrg, 'slv_stp', np.inf)
    obj.logger.info(' solve_ivp max_step= %s', str(max_stp))
    ret, m_dot_in_act = storage.clc_state_mstp(T_in, p_in, t_span, m_dot_H2_strg, m_dot_H2_grid, dm_prd, dm_cns,#dm_in, dm_out,
                                        max_step=max_stp, heatloss_wall=True, simu_obj=obj, t_dmn=obj.pstrg.t_dmn)
    obj.logger.info(' assign results to df ')
    full_df = full_df.assign(rho_strg=ret[0],
                    ...
                    P_ext=ret[10], #kW
                    P_ext_sc=ret[11], #kW
                    dm_H2_strg_act=ret[12],   # kg/s
                    #dm_H2_to_strg=dm_in,        # kg/s
                    dm_H2_strg=m_dot_H2_strg,     # kg/s
